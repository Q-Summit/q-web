...
`before_send` in `config.ts` drops exceptions the site cannot act on, grouped by why they are noise. `EXCEPTION_NETWORK_NOISE` covers per-engine fetch-layer failures on flaky or content-blocked mobile networks, which otherwise include the SDK recapturing its own failed beacons. `EXCEPTION_BROWSER_NOISE` covers WebKit reporting a skipped native view transition as an unhandled rejection: nothing in the codebase touches the view-transition API, so no script ever holds the promise WebKit rejects, and every iOS browser files it on ordinary navigations. `EXCEPTION_THIRD_PARTY_NOISE` covers code the site never shipped, meaning the cross-origin `Script error.` placeholder and Chrome for iOS's injected `__gCrWeb` bridge; the first is only safe to drop because the site loads no cross-origin scripts, so adding one means revisiting it.

Patterns are anchored on the exact message wherever the wording is stable, so a real error that merely contains one of those phrases still reports. The bar for a new entry is an exception with no stack that no change to this codebase could fix. Asset-delivery failures such as `Importing a module script failed.` deliberately stay visible: a bad build or a truncated upload surfaces exactly that way, so filtering them would hide an outage.
