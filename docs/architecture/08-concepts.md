...
- **Agent / maintainer propose:** `POST /api/content-sync` may upsert **draft versions only** as the operator's own Workspace user (stamped `@agent.q-summit.com` so the changelog reads "(agent)"); it never publishes or deploys ([section 6](06-runtime.md), [`../dev/content-sync.md`](../dev/content-sync.md)).
...
| `/llms.txt` page links | same | Each page `metaDescription` + curated route table in `llms.ts` |
| `/llms-full.txt` | `apps/web/src/pages/llms-full.txt.ts` via `llms.ts` | Main page sections from the content layer (not every nested block; no legal HTML) |
| JSON-LD | `apps/web/src/components/seo/JsonLd.astro` + page usage | Real content fields only; no invented facts |

Page SEO is `title` + `metaDescription` via `seoFields()`. Site Settings → `llms` holds AI identity only ([llmstxt.org](https://llmstxt.org/) index + community `/llms-full.txt`). Route lists in `llms.ts` stay hand-curated; update that table in the same PR as a new public Astro route.
