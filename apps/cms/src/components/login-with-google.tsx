...
// `.qs-login-intro--google ~ .login__form`, which depends on beforeLogin
// rendering as a sibling before LoginForm -- and on the marker class existing
// only when the Google env is set, so a developer without it keeps a usable form.
export const LoginWithGoogle: React.FC = () => {
  ...
  return (
    <div className="qs-login-intro qs-login-intro--google">
      <h1 className="qs-login-title">Q-Summit CMS</h1>
      <p className="qs-login-lede">Sign in with your q-summit.com account.</p>
      ...
      <p className="qs-login-note">Workspace accounts only.</p>
    </div>
  );
};
