...
- A `VRT_PAT` repo secret, a fine-grained token with `contents: write` on this repo (AUTO mode: the job commits refreshed baselines onto the PR branch, and `visual-propagate` re-runs updated PRs; the default `GITHUB_TOKEN` cannot re-trigger CI). AUTO commits use the PR contributor as **author** (so squash merges do not list the bot as a co-author) and `github-actions[bot]` as **committer** (botcheck skips that synchronize). After the first AUTO push, confirm GitHub shows committer `github-actions[bot]` (the push step sets the bot noreply email; the PAT only authenticates). If committer attribution is wrong, the visual job will re-run on every baseline push and may loop; the workflow emits a warning when it detects that shape.
...
1. **Media first.** `make propose` uploads local package `media/` binaries (or `--media-dir`) through `POST /api/content-sync/media` before applying the package. That path is create-if-missing only: an existing filename is skipped, never overwritten. Apply still resolves upload fields by filename (`resolveMediaId` in `apps/cms/src/content-sync/apply-package.ts`); it never fetches a URL. A referenced file that is neither already in Payload nor present locally still lands the doc in `skipped` with `missing media filename`. Admin bulk upload remains the fallback.
