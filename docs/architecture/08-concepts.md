...
- **Join Q speaker cards** take portrait crop (`crop.x` / `y` / `zoom` / `shiftY`) and the optional location-card badge (`kickoff.ui.registerLabel` + `registerHref`) from `page-kickoff`. An empty label hides the pill. A URL turns the pill into an outbound link; no third-party embed. Code must not key crops or copy to speaker names.
- **Join Q quiz Notion links** live on `page-kickoff` `quiz.results.notionHref`. The site attaches them to the hardcoded quiz team names (`Corporate`, `Startup & Venture Capital`, `Speaker`, `Growth & Partnerships`, and the rest of `TEAM_ORDER`). CMS labels such as `Corporate Relations` or `Startup & VC` are aliases of those names, not separate teams.
