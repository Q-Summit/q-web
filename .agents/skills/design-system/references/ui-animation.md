# UI animation (vendored reference)

Verbatim copy of the `ui-animation` skill from
[LukasStrickler/ai-dev-atelier](https://github.com/LukasStrickler/ai-dev-atelier/tree/main/content/skills/ui-animation),
minus its skill frontmatter. Upstream also ships
[`references/recipes.md`](https://github.com/LukasStrickler/ai-dev-atelier/blob/main/content/skills/ui-animation/references/recipes.md),
which is not vendored here because it is almost entirely Motion and React.

## What applies to q-web, and what does not

q-web is a static Astro site: no React, no Tailwind, no client-side animation
runtime, and per [ADR-0001](../../../../docs/decisions/0001-astro-static-site.md)
no SSR. Adding Motion, Framer, or `tailwindcss-animate` would be a new runtime
dependency and needs an ADR, so treat every Motion and Tailwind snippet below as
background reading, not as a pattern to copy.

| Upstream section | Status in q-web |
| --- | --- |
| Core Principles, Easing, Duration Guidelines | **Adopted** as `MOTION-1`. Encoded in DESIGN.md's Motion section and the `--ease-*` / `--duration-*` tokens. |
| Accessibility (`prefers-reduced-motion`, focus timing, touch targets) | **Adopted**, in the CSS form: media queries, not `useReducedMotion()`. `MOTION-2`. |
| Performance golden rules (`transform` and `opacity` only) | **Adopted.** `MOTION-3`. |
| Anti-patterns | **Adopted**, minus the Tailwind-specific rows and the `linear` row: here `linear` is correct for loops and for scroll-driven timelines (see DESIGN.md Motion), and wrong only for click-triggered transitions. |
| Enter/Exit, AnimatePresence | **Replaced.** q-web gets the same effect from `@view-transition` and `@starting-style` in `global.css`, and the scroll reveal in `ui/Timeline.astro` (`MOTION-5`). |
| Spring Animations, Layout Animations, `layoutId`, Gestures, tailwindcss-animate | **Not applicable.** They require Motion or Tailwind. Do not introduce either (`MOTION-4`). |
| "Integration with Other Skills" table | **Not applicable.** Those skills are the atelier's, not this repo's; use `.agents/skills/` here. |

Relative paths and tool names in the body below (`references/recipes.md`, `ui_to_artifact`, `ui_diff_check`) are upstream's and do not resolve here; only the table above applies.

Two of the three "Pro Curves" below are already this repo's tokens:
`--ease-snappy` is upstream's Snappy, `--ease-emphasized` is upstream's
Emphasized. That is the alignment worth preserving when editing motion.
...
- [`references/recipes.md`](https://github.com/LukasStrickler/ai-dev-atelier/blob/main/content/skills/ui-animation/references/recipes.md) (upstream, not vendored here): copy-paste patterns, integration examples, detailed traps
