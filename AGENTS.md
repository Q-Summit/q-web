...
| Visual regression (`*.vrt.ts`, baselines, advisory CI) | [`docs/dev/visual-testing.md`](docs/dev/visual-testing.md) |
| Other maintainer how-tos | [`docs/dev/`](docs/dev/) |
| Analytics (PostHog client, event taxonomy, MCP) | [`docs/dev/analytics.md`](docs/dev/analytics.md) |
...
- Mermaid via **docs-diagrams**; agent guide edits via **agent-files**; SEO/llms/social via **discoverability**; analytics events and PostHog via **posthog-analytics**; styling, tokens, and `ui/` primitives via **design-system** (`.agents/skills/`). **design-system** covers `apps/web` only; CMS admin styling follows Payload's tokens per [`apps/cms/DESIGN.md`](apps/cms/DESIGN.md).
- Pin GitHub Actions to a full commit SHA with a trailing `# vX.Y.Z` comment.
- New workflows: least-privilege `permissions:`. Blocking quality gates extend `pnpm run check`, never a parallel required workflow. The one deliberate exception is the advisory visual review (`.github/workflows/visual.yml`, [ADR-0007](docs/decisions/0007-visual-regression-testing.md)): browser-heavy, never required, never part of `pnpm run check`. Treat workflow-lint findings as blocking even when path-filtered.
