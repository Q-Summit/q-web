#!/usr/bin/env bash
# One-time setup of the label scheme (see .github/ISSUE_TEMPLATE/ and the
# scheduled workflows that file tracking issues: links.yml with "bug",
# cms-build.yml with "area: infra").
...
# Same contract as .github/admin/setup-repo.sh: explicit target repo, so a run from
