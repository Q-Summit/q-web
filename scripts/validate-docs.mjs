...
const SKIP_DIRS = new Set(['node_modules', '.git', 'dist', '.astro', '.next', '.vercel', '.wrangler']);
// Legal text is never restyled, so the style rules do not apply to it.
const DASH_EXEMPT = new Set(['LICENSE.md', 'LEGAL.md']);
...
function blankCodeSpans(line) {
  ...
  for (let i = 0; i < runs.length; i++) {
    const j = runs.findIndex((r, k) => k > i && r[0].length === runs[i][0].length);
    if (j === -1) continue;
  ...
  mdLines(fs.readFileSync(abs, 'utf8')).forEach(({ line, inCode }, i) => {
    ...
    for (const match of line.replace(/\\\\/g, '  ').matchAll(LINK)) {
      ...
      if (!fs.existsSync(resolved)) {
        err(rel(abs), `line ${i + 1}: relative link "${raw}" does not resolve to an existing file`);
      }
    }
  });
}
...
const hasImportLine = (abs) =>
  mdLines(fs.readFileSync(abs, 'utf8')).some(({ line, inCode }) => !inCode && line.trim() === '@AGENTS.md');
for (const abs of allFiles.filter((f) => path.basename(f) === 'AGENTS.md')) {
  const sibling = path.join(path.dirname(abs), 'CLAUDE.md');
  if (!fs.existsSync(sibling)) err(rel(abs), 'has no sibling CLAUDE.md (must exist and import @AGENTS.md)');
  else if (!hasImportLine(sibling)) err(rel(sibling), 'must contain a line that is exactly "@AGENTS.md" (outside code fences)');
}
...
try {
  ...
  } else if (fs.realpathSync(link) !== fs.realpathSync(path.join(root, '.agents', 'skills'))) {
    err('.claude/skills', 'symlink does not resolve to .agents/skills: run "pnpm run setup"');
  }
} catch {
...
const adrIndexAbs = path.join(root, 'docs', 'architecture', '09-architecture-decisions.md');
const adrIndex = fs.existsSync(adrIndexAbs) ? fs.readFileSync(adrIndexAbs, 'utf8') : '';
if (fs.existsSync(adrDir)) {
  for (const name of fs.readdirSync(adrDir)) {
    ...
    if (!/^- \*\*Status:\*\* /m.test(text)) err(fileRel, 'missing the "- **Status:**" bullet (see _template.md)');
    if (!/^- \*\*Date:\*\* \d{4}-\d{2}-\d{2}/m.test(text)) err(fileRel, 'missing the "- **Date:** YYYY-MM-DD" bullet (see _template.md)');
    if (!adrIndex.includes(name)) {
      err('docs/architecture/09-architecture-decisions.md', `has no index row linking ${fileRel} (same-PR rule)`);
    }
  }
}
...
for (const abs of mdFiles) {
  ...
  mdLines(raw).forEach(({ line, inCode }, i) => {
    if (DASH.test(rawLines[i])) {
      err(fileRel, `line ${i + 1} contains an em or en dash: rewrite with a comma, colon, period, or parentheses (house style, see AGENTS.md)`);
      return;
    }
    if (inCode) return;
    ...
    if (/\S\s+-\s+\S/.test(prose)) {
      err(fileRel, `line ${i + 1} uses a spaced hyphen as dash punctuation (" - "): rewrite with a comma, colon, period, or parentheses; hyphens joining words ("Same-PR") are fine (house style, see AGENTS.md)`);
    }
  });
}
...
const TEXT_EXT = /\.(ya?ml|jsonc?|toml|mjs|cjs|js|ts|tsx|astro|css|html|svg|sh|txt|env|example)$/i;
for (const abs of allFiles.filter((f) => !f.endsWith('.md'))) {
  const fileRel = rel(abs);
  const base = fileRel.split('/').pop();
  const ext = base.includes('.') && !base.startsWith('.') ? base.slice(base.indexOf('.')) : '';
  ...
  buf
    ...
    .split('\n')
    .forEach((line, i) => {
      if (DASH.test(line)) err(fileRel, `line ${i + 1} contains an em or en dash (house style, see AGENTS.md)`);
    });
}
