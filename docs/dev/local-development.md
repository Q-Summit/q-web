...
Set `SITE_URL=http://localhost:4321` in `apps/cms/.env` so Live Preview iframes the local Astro site (that is the default if unset). Production sets `SITE_URL=https://q-summit.com` on Vercel and `PUBLIC_CMS_URL=https://cms.q-summit.com` on the site build. In the Payload admin, open any **Website pages** global → click the **Live Preview** toggle (eye icon) so the side-by-side iframe appears; nodes marked `data-lp` update as you type. Collection grids (speakers, FAQs, partners) still come from published build data and do not live-update from a page-global edit.
