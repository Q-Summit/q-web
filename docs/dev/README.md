...
| [`go-live.md`](go-live.md) | Production CF (R2 + Worker), Vercel connect + migrations, Google SSO setup, admin bootstrap, publish → deploy hook, PostHog turn-on, and keys that exist today |
| [`analytics.md`](analytics.md) | PostHog cookieless client reference: how it works, what is collected, env vars, troubleshooting, MCP server auth, CI gates |
