...
flowchart LR
  ...
  subgraph platform["q-web"]
    ...
    R2 -.->|"delivers images, video"| SITE
    CMS -.->|"publish triggers rebuild"| SITE
  end

  SITE -.->|"usage events, cookieless"| PH["PostHog Cloud EU"]
  SITE -.->|"ticket links"| TIX["vivenu ticketing<br/>(external)"]
...
_Visitors only browse the public site; HTML comes from Cloudflare's edge and media from R2. The CMS and database sit behind the build step, so a CMS or database outage never takes the site down. Dotted edges are external or asynchronous._
...
- **Editors never touch the repo.** All content flows through Payload; the publish-to-rebuild flow is [section 6](06-runtime.md).
- **Analytics is cookieless.** The site sends anonymous usage events to PostHog Cloud EU; no cookies, no consent banner ([ADR-0003](../decisions/0003-posthog-cookieless-analytics.md)).
