...
- **Join Q speaker cards** take portrait crop (`crop.x` / `y` / `zoom` / `shiftY`) and the optional location-card badge (`kickoff.ui.registerLabel`) from `page-kickoff`. An empty badge hides the pill. Code must not key crops or copy to speaker names.
